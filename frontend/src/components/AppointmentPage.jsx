...
import { CalendarDays, CreditCard, Wallet, CheckCircle, Bell, Clock, XCircle } from 'lucide-react';
...
const AppointmentPage = () => {
            ...
            if (user?.id) {
                try {
                    ...
                    const doctors = arr.filter(
                        (a) => (a.doctorId !== undefined && a.doctorId !== null) || !!a.doctorName || !a.serviceId,
                    );
                    setDoctorAppts(doctors);
                    setAppointmentsRaw((p) => ({ ...p, doctors }));
                } catch (err2) {
                    console.error("Debug fallback failed (doctors):", err2?.response?.data || err2.message || err2);
                    setError((prev) => prev ? prev + " | Doctors failed" : "Failed to load doctor appointments. Check console.");
                    setDoctorAppts([]);
                }
            } else {
                setError((prev) => prev ? prev + " | No user id for doctors" : "Failed to load doctor appointments and no user id available for debug fallback.");
                setDoctorAppts([]);
            }
        } finally {
        ...
        try {
            ...
            if (user?.id) {
                try {
                    ...
                    setError((prev) => prev ? prev + " | Services failed" : "Failed to load service appointments. Check console.");
                    setServiceAppts([]);
                }
            } else {
                setError((prev) => prev ? prev + " | No user id for services" : "Failed to load service appointments and no user id available for debug fallback.");
                setServiceAppts([]);
            }
        } finally {
    ...
    function normalizeRescheduled(rt) {
        ...
        return {
            ...
            time: rt.time || (rt.hour ? `${rt.hour}:${pad(rt.minute || 0)} ${rt.ampm || ""}` : rt?.timeString || ""),
        };
    }
    ...
    const appointmentData = useMemo(() => {
        return doctorAppts
            .map((a) => {
                ...
                const image =
                    doctorObj.imageUrl || doctorObj.image || doctorObj.avatar ||
                    a.doctorImage?.url || a.doctorImage || "";
                const doctorName =
                ...
                return { id, image, doctor: doctorName, patientName, specialization, experience, date, time, payment, status, rescheduledTo };
            })
    ...
    const serviceData = useMemo(() => {
        return serviceAppts
            .map((s) => {
                ...
                const image =
                    svc.imageUrl || svc.image || svc.imageSmall ||
                    s.serviceImage?.url || s.serviceImage || "";
                ...
                const status = s.status || (s.payment && s.payment.status === "Paid" ? "Confirmed" : "Pending");
