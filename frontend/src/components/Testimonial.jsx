...
const Testimonial = () => {
    ...
    const [isPausedLeft, setIsPausedLeft] = useState(false);   // ✅ separate pause states
    ...
    // ✅ separate isPausedLeft and isPausedRight in dependency array
   useEffect(() => {
    ...
    let rafId;
    let initialized = false;  // ✅ track if right column is initialized

    const smoothScroll = () => {
        // ✅ initialize right column on first frame when scrollHeight is available
        if (!initialized && scrollRight.scrollHeight > 0) {
        ...
        if (!isPausedRight && initialized) {  // ✅ only scroll right after initialized
    ...
    const renderStars = (rating) =>
        Array.from({ length: 5 }, (_, i) => (
            <span
                key={i}
                className={i < rating ? testimonialStyles.activeStar : testimonialStyles.inactiveStar}
            >
    ...
    return (
        <div className={testimonialStyles.container}>
            ...
            {/* ✅ removed onMouseEnter/onMouseLeave from grid wrapper */}
            <div className={testimonialStyles.grid}>
                ...
                <div className={`${testimonialStyles.columnContainer} ${testimonialStyles.leftColumnBorder}`}>
                    ...
                    {/* ✅ removed duplicate ref div, hover handlers on this column only */}
                    <div
                        ref={scrollRefLeft}
                ...
                <div className={`${testimonialStyles.columnContainer} ${testimonialStyles.rightColumnBorder}`}>
                    ...
                    {/* ✅ hover handlers on this column only */}
                    <div
                        ref={scrollRefRight}
                        className={testimonialStyles.scrollContainer}
