...
// this function will parse the time and return the am or pm
...
export const createServiceAppointment = async (req, res) => {
    try {
        ...
        if (!clerkMiddleware) return res.status(401).json({
            success: false,
...
export const getServiceAppointments = async (req, res) => {
    try {
        ...
        .popluate("serviceId","name image imageUrl imageSmaill")
    }
    catch (err) {
