    ...
    try {
        ...
        if (payload.role && payload.role !== "doctor") {
            return res.status(403).json({
                success: false,
                message: "Accesss Denied (not a doctor)"
            });
        }
        // Fetch doctor
        const doctor = await Doctor.findById(payload.id).select("-password");//excluding password
        ...
        console.error("Doctor JWT Verfication failed: ", err);
        return res.status(401).json({
            success: false,
